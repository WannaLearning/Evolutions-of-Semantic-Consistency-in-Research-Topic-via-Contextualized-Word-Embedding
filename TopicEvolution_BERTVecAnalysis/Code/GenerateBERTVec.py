...
def GenerateTopicEmbeddings():
    ...
    # 数据路径: file_content_i(摘要), file_fos_i(所有主题), filtered_FoSs(待分析的主题)
    ...
    # 向量存储路径:
    vec_path_i     = os.path.join(abs_file_path, "vec_{}.pkl")
    ...
    for i in range(0, 17):
            ...
            while True:
                ...
                if oneline:
                    ...
                    if len(title) == 0 and len(abstract) == 0:
                        continue
                    content  = title + "." + abstract         # read title + abstract
        ...
        save_file(results, vec_path_i.format(i))
        del pid2content, pid2FoSs, results 
...
def SaveEmbeddingsSeperately():
    ...
    for i in range(0, 17):
        # 第i块的向量信息
        vec_path_i = os.path.join(abs_file_path, "vec_{}.pkl")
        vec_path_i = vec_path_i.format(i)
        vecs = read_file(vec_path_i)
        # 分词存放
